...
int set_task_isolation(int flags)
{
	int rc, retries;
...
static int run_test(void (*setup_func)(), int (*test_func)(), int flags)
{
	...
	if (setup_func)
		setup_func();
	if (flags && set_task_isolation(flags) != 0)
		ksft_exit_fail();
	rc = test_func();
	exit(rc);
}
...
static int do_syscall_multi(void)
{
	...
	rc = write(STDOUT_FILENO, msg1, strlen(msg1));
	assert(rc = strlen(msg1));
	rc = write(STDOUT_FILENO, msg2, strlen(msg2));
	assert(rc = strlen(msg2));
	return KSFT_PASS;
}
...
static int do_quiesce(void)
{
	...
	if (set_task_isolation(PR_TASK_ISOLATION_ENABLE |
			       PR_TASK_ISOLATION_NOSIG) != 0)
		goto fail;
	*statep = 1;
